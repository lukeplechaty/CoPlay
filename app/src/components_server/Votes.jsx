...
export default function Votes({ video_id, initialUpvotes = 0, initialDownvotes = 0, initialUserVote = null, userId }) {
  ...
  async function handleVote(direction) {
    ...
    try {
      ...
      const unvote = (direction === 'up' && userVote === true) || (direction === 'down' && userVote === false);
      const res = await fetch('/api/vote', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ video_id, direction: unvote ? null : direction }),
      });
