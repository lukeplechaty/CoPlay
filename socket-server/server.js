...
io.on("connection", (socket) => {
  ...
  socket.on("join-room", (room_id) => {
    ...
    console.log(
      ...
      Array.from(io.sockets.adapter.rooms.get(room_id) || [])
    );
    const sockets_in_room = Array.from(
  ...
  socket.on("request_video_status", ({ room_id, requester_socket_id }) => {
    console.log(
      `[Server] Received request_video_status from ${requester_socket_id} for room ${room_id}`
    );
    const sockets_in_room = Array.from(
      io.sockets.adapter.rooms.get(room_id) || []
    );
    ...
    if (host_id && requester_socket_id) {
      io.to(host_id).emit("request_video_status", {
        room_id,
        requester_socket_id,
      });
      console.log(
        `[Server] Forwarded request_video_status from ${requester_socket_id} to host ${host_id} in room ${room_id}`
      );
    }
  });
  ...
  socket.on(
    ...
    ({ room_id, status, target_socket_id }) => {
      console.log(
        `[Server] Received video_status_response from host ${socket.id} for room ${room_id}, sending to ${target_socket_id}`
      );
      if (target_socket_id) {
        io.to(target_socket_id).emit("video_status_update", { status });
        console.log(
          `[Server] Host ${socket.id} sent initial video_status_update to ${target_socket_id} in room ${room_id}:`,
          status
        );
      }
    }
  ...
  socket.on("video_status_update", ({ room_id, status }) => {
    ...
    console.log(
      `[Server] Broadcasted video_status_update to room ${room_id}:`,
      status
    );
  });
  ...
  socket.on("video_status_update", ({ room_id, status, target_socket_id }) => {
    if (target_socket_id) {
      io.to(target_socket_id).emit("video_status_update", { status });
      console.log(
        `[Server] Host ${socket.id} sent video_status_update to ${target_socket_id} in room ${room_id}:`,
        status
      );
    } else {
      socket.to(room_id).emit("video_status_update", { status });
  ...
  socket.on("chat_update", ({ room_id, message }) => {
    ...
    console.log(`[Server] Sockets in room ${room_id}:`, sockets_in_room);
    if (room_id) {
      socket.to(room_id).emit("chat_update", { message });
      console.log(
        `[Server] Broadcasted chat_update to room ${room_id}:`,
        message
      );
    }
  });
